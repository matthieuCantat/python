	...
	def deform( self, dataBlock, iter, matrix, index ):
		...
		if envelopeValue!=0 and not inMesh.isNull() and not targetMesh.isNull():
			...
			while not iter.isDone():
				...
				"""if index == 789:
				    print self.faceIndexArray.length()"""
			...
			self.fallOffActiveArray = om.MIntArray(self.hasBeenCollide)
			
			self.fallOffArrayTmp = om.MIntArray(self.fallOffActiveArray)
			
			hasBeenComp = om.MIntArray(self.hasBeenCollide)
			
			vtxDist = om.MDoubleArray(self.fallOffActiveArray.length(), 0)
			...
			while not polyIter.isDone():
			    ...
			    vtx = self.vtxPerPoly[index]
			    
			    for v in xrange(vtx.length()):
			        ...
			        if self.hasBeenCollide[currentVtx] == 1 and hasBeenComp[currentVtx] == 1:
			            continue
			        
			            
			        for i in xrange(vtx.length()):
			            
			            targetVtx = vtx[i]
			            
			            if i == v or self.hasBeenCollide[targetVtx] == 0 or hasBeenComp[targetVtx] == 0:
			                continue
			            
			            # getting distance between face's points
                    
			            distTmp = (self.inMeshPoints[currentVtx].distanceTo(self.inMeshPoints[targetVtx]))
			            
			            if distTmp < dist:
			                
			                dist = distTmp
			                attractVtx = targetVtx
			                
			                if self.hasBeenCollide[targetVtx] == 1:
			                    vtxDist[attractVtx] = 0.0
			        ...
			        vtxDist[v] = (1 - vtxDist[attractVtx]) - dist
			        ...
			        magnitude = radius / vtxDist[v]
			        
			        newPos = self.inMeshPoints[currentVtx] + (self.inMeshPoints[targetVtx] - self.newPoints[currentVtx]) * magnitude
			        
			        self.fallOffActiveArray[currentVtx] = 1
			        hasBeenComp[currentVtx] = 1

			        self.newPoints[currentVtx].x = newPos.x
			        self.newPoints[currentVtx].y = newPos.y
			        self.newPoints[currentVtx].z = newPos.z
...
def nodeInitializer():
	...
	stickyDeformer.relaxFactor2Att = nAttr.create("relaxFactor2", "reFact2", om.MFnNumericData.kDouble, 0.1)
	nAttr.setMin(0.001)
	nAttr.setMax(.999)
	nAttr.setKeyable(True)
	...
	stickyDeformer.fallOff = nAttr.create("fallOff", "fall", om.MFnNumericData.kDouble, 0.0)
	nAttr.setMin(0.0)
	nAttr.setKeyable(True)
