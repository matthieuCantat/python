...
def copySkinNurbsToMesh():
	
	timeValues = []
	
	timeValues.append(time.clock())
                                                                                                                                       
	selection = getNurbsAndMeshFromSelection()
	nurbs = selection[0]
	meshs = selection[1]
	
	joints = getJointMultipleSkins(nurbs)                                                                                              
	...
	for mesh in meshs:                                                                                                                 
		...
		UVequi = getUvEquivalentToVertex( mesh , nurbs ) 
		
		timeValues.append(time.clock())
		
		UVcoords = getUVcoords( UVequi )                                                                                      
		...
		for joint in joints:                                                                                                           
			...
			moveJoint( nurbs , joint , distMove , vector , 0 )                                                                                                                                                                                           
			UVcoordsMove = getUVcoords( UVequi )                                                                                                                                                                                        
			moveJoint( nurbs ,  joint , distMove , vector , 1 )                                                                                         
                                                                                                                
			UVdists = getDistanceArray( UVcoords , UVcoordsMove )                                                                      
                                                                                                                  
			inf[joint] = convertDistanceToInfs( UVdists , distMove)                                                                           
	...
	for mesh in meshs:  		
		...
		timeValues.append(time.clock())	
		newSkinCluster = makeSkinCluster( mesh ,joints , infMesh[mesh] )		
		timeValues.append(time.clock())
		
	printRapport( timeValues )
	print( '===== DONE =====' )
